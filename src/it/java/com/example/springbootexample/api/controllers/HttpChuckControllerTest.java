...
class HttpChuckControllerTest extends IntegrationTestParent {
  ...
  void givenHttpCall_whenGetChuckNorrisJoke_thenReturnJoke() throws Exception {
    ...
    this.mockMvc.perform(get("/chuck")).andExpect(status().isOk()).andExpect(content().string("{\"joke\":\"A_JOKE\"}"));
  }

  @Test
  void givenHttpCall_whenExternalServiceInError_thenReturnHttp500() throws Exception {
    mockServer.expect(requestTo(configService.getChuckJokeUrl() + "/jokes/random")).andRespond(withServerError());

    this.mockMvc.perform(get("/chuck")).andExpect(status().isInternalServerError());
  }
}
