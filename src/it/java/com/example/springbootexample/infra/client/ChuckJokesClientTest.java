...
class ChuckJokesClientTest extends IntegrationTestParent {
  ...
  void whenPing_thenHttpCallWasMade() {
    mockServer.expect(requestTo(configService.getChuckJokeUrl() + configService.getChuckJokeHealthEndpoint())).andRespond(withSuccess());
  ...
  void givenHttpError_whenGetJoke_thenThrowsChuckNorrisException() {
    ...
    Assertions.assertThrows(ChuckNorrisException.class, () -> chuckJokesClient.getJoke());
  }
}
